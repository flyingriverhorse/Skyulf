...
class EDAAnalyzer:
                ...
                if val > 10.0:
                    alerts.append(
                        Alert(
                            ...
                            message=f"Column '{col}' has very high VIF ({val:.1f}). Consider removing it.",
                            severity="warning",
                        )
                    )
                elif val > 5.0:
    ...
    def _generate_recommendations(
        self, profiles: Dict[str, ColumnProfile], alerts: List[Alert], target_col: Optional[str]
    ) -> List[Recommendation]:
        recs = []
    ...
    def _analyze_column(self, col: str) -> Tuple[ColumnProfile, List[Alert]]:
        ...
        profile = ColumnProfile(
            name=col, dtype=semantic_type, missing_count=null_count, missing_percentage=null_pct
        )
                ...
                if profile.categorical_stats.unique_count == self.row_count:
                    ...
                    alerts.append(
                        Alert(
                            ...
                            message=f"Column '{col}' has high cardinality ({profile.categorical_stats.unique_count}).",
                            severity="warning",
                        )
                    )
    ...
    def _discover_rules(
        self, feature_cols: List[str], target_col: str, task_type: Optional[str] = None
    ) -> Optional[RuleTree]:
        """
        ...
        try:
            ...
            cat_cols = [
                ...
                if self._get_semantic_type(df_sample[c]) in ["Categorical", "Boolean", "Text"]
            ]
    ...
    def _prepare_matrix_sample(
        self, numeric_cols: List[str], target_col: Optional[str] = None, limit: int = 5000
        ...
        try:
            ...
            if self.row_count > limit:
                sample_df = self.df.select(cols_to_fetch).sample(
