            ...
            for op in operations:
                ...
                if op_type == "trim":
                    ...
                    if mode == "keep_alphanumeric":
                        ...
                        series = series.str.replace(r"[^a-zA-Z0-9]", replacement, regex=True)
                    elif mode == "keep_alphanumeric_space":
                        # Keep letters, numbers, and spaces
                        series = series.str.replace(r"[^a-zA-Z0-9\s]", replacement, regex=True)
                    elif mode == "letters_only":
                        series = series.str.replace(r"[^a-zA-Z]", replacement, regex=True)
                    elif mode == "digits_only":
            ...
            for col in valid_cols:
                ...
                if rule == "negative":
                    ...
                    if min_value is not None and max_value is not None:
                        expr = (
                            pl.when((expr < min_value) | (expr > max_value))
                            .then(final_replacement)
                            .otherwise(expr)
                        )
        ...
        if engine.name == EngineName.POLARS:
            ...
            if mapping:
                ...
                if is_nested:
                    ...
                    for col in valid_cols:
                        if col in mapping:
                            ...
                            exprs.append(
                                pl.col(col).replace(map_dict, default=pl.col(col)).alias(col)
                            )
                else:
                    # Global mapping
                    for col in valid_cols:
                ...
                if to_replace is not None:
                    for col in valid_cols:
                        if isinstance(to_replace, (dict, pd.Series)) or hasattr(
                            to_replace, "items"
                        ):
                            exprs.append(
                                pl.col(col).replace(to_replace, default=pl.col(col)).alias(col)
                            )
                        else:
                            ...
                            exprs.append(
                                pl.col(col)
                                .replace({to_replace: value}, default=pl.col(col))
                                .alias(col)
                            )
