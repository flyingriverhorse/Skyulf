...
- **Modal Accessibility:** All 6 modals (DatasetPreview, AddSource, IngestionJobs, VariableDetail, JobsHistory, StrategySettings) now have `role="dialog"`, `aria-modal="true"`, `aria-labelledby` linked to their title, `aria-label="Close"` on close buttons, and Escape key / backdrop click to dismiss.
