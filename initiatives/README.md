...
| [dual-engine-correctness/](dual-engine-correctness/README.md) | Whether the Polars/Pandas dual-engine design is correct end-to-end (engine → nodes → modeling → persistence → inference → monitoring) and whether the leakage claim holds. Four parallel Opus-5 agents plus an adversarial rubber-duck pass, every finding reproduced with an executable probe. 33 findings (3 CRITICAL, 12 HIGH, 12 MED, 6 LOW); Polars is architecturally sound and the numpy handoff is bit-identical, but 6 correctness bugs block a parity claim and 3 CRITICAL inference bugs affect both engines. All pre-existing — coverage gaps, not regressions. | Investigation complete, tiered fix plan ready, **no fixes applied** |
