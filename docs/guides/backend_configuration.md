...
| `SECURITY_HEADERS` | *(production policy)* | Production-only JSON object of HTTP response headers; replaces the default map. `{}` disables injection |
...
Production responses carry the configured MIME-sniffing, framing, HSTS and
Content Security Policy headers. The middleware covers normal responses,
redirects, static files, streaming bodies and errors returned by Skyulf's
error middleware. Configured values take precedence over an endpoint's value
for the same header; unrelated headers, cookies and response bodies are preserved.
Development and testing profiles do not add this production policy.

The default CSP allows the resources used by Canvas image exports and map
tiles, plus Swagger/ReDoc assets when API docs are enabled. It retains inline
script/style support for the existing UI and does not enable `unsafe-eval`.
Use `SECURITY_HEADERS` to supply the complete header map for a different
deployment policy.

CORS remains the outermost application middleware. Its preflight responses
and fallback responses generated by Starlette's outer server-error handler
do not pass through this header middleware. WebSocket traffic is passed
through unchanged.
