...
def generate_geo_data(n_points=100):
    ...
    return pl.DataFrame(
        {"store_id": range(n_points), "latitude": lats, "longitude": lons, "revenue": revenue}
    )
...
def visualize_map(df, profile):
    ...
    # Plot all points
    plt.scatter(
        df["longitude"], df["latitude"], c=df["revenue"], cmap="viridis", alpha=0.7, label="Stores"
    )
...
def main():
    ...
    if profile.geospatial:
        ...
        table.add_row(
            "Latitude Range",
            f"{profile.geospatial.min_lat:.4f} to {profile.geospatial.max_lat:.4f}",
        )
        table.add_row(
            "Longitude Range",
            f"{profile.geospatial.min_lon:.4f} to {profile.geospatial.max_lon:.4f}",
        )
        table.add_row(
            "Centroid",
            f"{profile.geospatial.centroid_lat:.4f}, {profile.geospatial.centroid_lon:.4f}",
        )
