...
from ..models import DataSourceResponse, DataSourceCreate, DataSourceUpdate
from ..exceptions import DataIngestionException, DataSourceNotFoundError, FileUploadError
from ...database.models import User, DataSource
...
async def delete_data_source(
    ...
    try:
        ...
        if not (is_admin or (owner_id is not None and current_user_id is not None and owner_id == current_user_id)):
            ...
            try:
                log_data_action("DELETE_DATA_SOURCE", False, f"Denied delete attempt for source '{source_name}' (ID: {source_id_int}) by {getattr(current_user, 'username', 'unknown')}")
            except Exception:
        ...
        if deletion_result:
            ...
            try:
                log_data_action("DELETE_DATA_SOURCE", True, f"Deleted source '{source_name}' (ID: {source_id_int}) by {getattr(current_user, 'username', 'unknown')}")
            except Exception:
...
async def bulk_delete_data_sources(
    ...
    try:
        ...
        logger.info(f"Bulk delete request by {current_user.username}: {len(source_ids)} sources")
        ...
        try:
            current_user_id = getattr(current_user, 'id', None) or await service._get_user_id_from_username(getattr(current_user, 'username', '') or '')
        except Exception:
            current_user_id = None
        ...
        for source_id in source_ids_int:
            try:
                ...
                if not (is_admin or (owner_id is not None and current_user_id is not None and owner_id == current_user_id)):
                    ...
                    try:
                        log_data_action("BULK_DELETE_DATA_SOURCES", False, f"Denied delete attempt for {source_name} (ID: {source_id}) by {getattr(current_user, 'username', 'unknown')}")
                    except Exception:
                ...
                if deleted:
                    ...
                    logger.info(f"Successfully deleted source {source_id} ({source_name}) for user {getattr(current_user, 'username', 'unknown')}")
                else:
...
async def get_health_status():
    ...
    return HealthResponse(
        ...
        timestamp=str(logger.handlers[0].formatter.formatTime(
            logger.handlers[0], 
            logger.makeRecord("data_health", 20, "", 0, "health_check", (), None)
        )) if logger.handlers else str(None),
        components={
            "file_upload": "healthy",
