...
_KNOWN_SPLITS: Tuple[str, ...] = ("train", "test", "validation")
_SPLIT_ORDER = {name: index for index, name in enumerate(_KNOWN_SPLITS)}


def _normalize_split_name(raw_name: Any) -> Tuple[str, str]:
...
def apply_transformer_audit(
    ...
    *,
    ...
    for record in records:
        ...
        split_entries: Dict[str, TransformerSplitActivitySignal] = {}
        if isinstance(split_activity_map, dict):
            for raw_split, payload in split_activity_map.items():
                ...
                if action_value not in {"fit_transform", "transform", "not_applied", "not_available"}:
                    action_value = "not_applied"
                try:
