...
def _extract_plan_from_metadata(metadata: Optional[Dict[str, Any]]) -> Tuple[List[int], List[str], List[Dict[str, Any]]]:
...
def apply_polynomial_features(
    ...
    if detail_flags:
        method_label = f"PolynomialFeatures (degree={config.degree}; {'; '.join(detail_flags)})"
    else:
        method_label = f"PolynomialFeatures (degree={config.degree})"
    ...
    if has_splits and storage is not None:
        ...
        train_numeric = numeric_frame.loc[train_mask]
        ...
        if train_rows > 0 and not train_numeric.empty:
            ...
            selected_indices, output_columns, feature_summaries = _prepare_feature_plan(
                transformer,
                input_columns=valid_columns,
                config=config,
                occupied=working_frame.columns,
            )

            metadata = {
                "input_columns": list(valid_columns),
            ...
            if not stored_indices or not stored_outputs:
                ...
                metadata.setdefault("column_summary", column_summary)
                metadata.setdefault("method_label", method_label)
        ...
        transformer.fit(numeric_frame.values)
        transform_mode = "fit"
        selected_indices, output_columns, feature_summaries = _prepare_feature_plan(
            transformer,
            input_columns=valid_columns,
    ...
    transformed_matrix = transformer.transform(numeric_frame.values)
    ...
    working_frame = pd.concat([working_frame, feature_frame], axis=1)
    ...
    if storage is not None and has_splits:
        train_rows = int(split_counts.get("train", 0))
        train_action = "fit_transform" if transform_mode == "fit" and train_rows > 0 else "transform"
    ...
    summary = (
        f"Polynomial features: added {len(output_columns)} column{'s' if len(output_columns) != 1 else ''} "
        f"(degree={config.degree}, columns={', '.join(valid_columns)})"
    )
