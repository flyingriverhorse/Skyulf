...
def build_data_snapshot_response(
    ...
    *,
    ...
    if column_count:
        for column_name in preview_frame.columns:
            ...
            if pd.api.types.is_integer_dtype(series):
                logical_family = "integer"
            elif pd.api.types.is_float_dtype(series):
                logical_family = "numeric"
            elif pd.api.types.is_bool_dtype(series):
                logical_family = "boolean"
            elif pd.api.types.is_categorical_dtype(series):
                logical_family = "categorical"
            elif pd.api.types.is_datetime64_any_dtype(series):
                logical_family = "datetime"
            elif pd.api.types.is_string_dtype(series):
                logical_family = "string"
            else:
                logical_family = "unknown"
    ...
    full_dataset_delta = estimated_full_dataset_rows - (baseline_total_rows if baseline_total_rows > 0 else preview_row_count)
