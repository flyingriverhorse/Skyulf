    ...
    if isinstance(raw, dict):
        for method_key, payload in raw.items():
            ...
            if method not in OUTLIER_METHODS:
                continue
            allowed_keys = OUTLIER_PARAMETER_KEYS[method]  # type: ignore[index]
            current = parameters[method]  # type: ignore[index]
...
def _normalize_outlier_config(config: Any) -> NormalizedOutlierConfig:
    ...
    default_method: OutlierMethodName = (
        raw_default_method if raw_default_method in OUTLIER_METHODS else OUTLIER_DEFAULT_METHOD
    ...
    if isinstance(raw_column_methods, dict):
        for column, method in raw_column_methods.items():
            ...
            if column_name and method_key in OUTLIER_METHODS:
                column_methods[column_name] = method_key  # type: ignore[assignment]
...
def _evaluate_elliptic_envelope(
    ...
    if affected_rows == 0:
        ...
        summary.notes.append(
            f"Flagged {affected_rows} of {total_rows} rows (~{summary.affected_ratio * 100:.2f}%)."
        )

    return summary, mask


def _summarize_manual(series: pd.Series, lower_bound: Optional[float], upper_bound: Optional[float]) -> OutlierMethodSummary:
    valid = series.dropna()
    total_rows = int(valid.shape[0])
...
def _apply_outlier_removal(
    ...
    for column in candidate_columns:
        ...
        if prepared is None:
            signal.skipped_columns.append(f"{column} (insufficient numeric data)")
            continue

        numeric_series, stats = prepared
        total_rows = int(numeric_series.dropna().shape[0])
        if total_rows == 0:
            signal.skipped_columns.append(f"{column} (no numeric rows)")
            continue
        ...
        applied_signal = OutlierAppliedColumnSignal(column=column, method=method, action="remove", total_rows=total_rows)
        ...
        transformer_status: Optional[str] = None  # "fit", "reuse", "direct"
        ...
        if storage is None:
            if method == "zscore":
                ...
                if summary.affected_rows and summary.lower_bound is not None and summary.upper_bound is not None:
                    mask_to_remove = (
                        (numeric_series < summary.lower_bound) | (numeric_series > summary.upper_bound)
                    ).reindex(working_frame.index, fill_value=False)
                ...
                if mask is not None and summary.affected_rows:
                    mask_to_remove = mask.reindex(working_frame.index, fill_value=False)
                ...
                if summary.lower_bound is not None and summary.upper_bound is not None:
                    ...
                    clipped_series = clipped
                    if summary.affected_rows:
            ...
            if method == "zscore":
                ...
                if requires_training and train_valid_values is not None and not train_valid_values.empty:
                    ...
                    if bounds is not None:
                        ...
                        applied_notes.append("Fitted thresholds on training split")
                    else:
                        skip_reason = "Unable to compute z-score thresholds from training data"

                if transformer_status != "fit" and stored_metadata is not None:
                    ...
                    if stored_lower is not None and stored_upper is not None:
                        ...
                        skip_reason = None
                        applied_notes.append("Reused stored thresholds")
                ...
                if transformer_status is None:
                    if skip_reason is None:
                        skip_reason = "Training split unavailable to fit z-score thresholds"

                if transformer_status is not None and mask_to_remove is not None:
                    affected_rows = int(mask_to_remove.sum())
                    applied_signal.action = "remove"
                    applied_signal.lower_bound = lower_bound
                    applied_signal.upper_bound = upper_bound
                    applied_signal.affected_rows = affected_rows
                    applied_notes.append(
                        f"Threshold={float(threshold):.4f}; bounds {_format_bound(lower_bound)} – {_format_bound(upper_bound)}"
                    )
                ...
                if requires_training and train_valid_values is not None and not train_valid_values.empty:
                    ...
                    if bounds is not None:
                        ...
                        applied_notes.append("Fitted IQR bounds on training split")
                    else:
                        skip_reason = "Unable to compute IQR bounds from training data"
                ...
                if transformer_status != "fit" and stored_metadata is not None:
                    ...
                    if stored_lower is not None and stored_upper is not None:
                        ...
                        transformer_status = "reuse"
                        skip_reason = None
                        applied_notes.append("Reused stored IQR bounds")
                        multiplier = float(stored_metadata.get("multiplier", multiplier))

                if transformer_status is None:
                    if skip_reason is None:
                        skip_reason = "Training split unavailable to fit IQR bounds"
                ...
                if transformer_status is not None and mask_to_remove is not None:
                    ...
                    applied_signal.affected_rows = affected_rows
                    applied_notes.append(
                        f"Multiplier={float(multiplier):.4f}; bounds {_format_bound(lower_bound)} – {_format_bound(upper_bound)}"
                    )
                ...
                if requires_training and train_valid_values is not None and not train_valid_values.empty:
                    ...
                    if bounds is not None:
                        ...
                        applied_notes.append("Fitted winsor bounds on training split")
                    else:
                        skip_reason = "Unable to compute winsor bounds from training data"
                ...
                if transformer_status != "fit" and stored_metadata is not None:
                    ...
                    if stored_lower is not None and stored_upper is not None:
                        ...
                        transformer_status = "reuse"
                        skip_reason = None
                        applied_notes.append("Reused stored winsor bounds")
                ...
                if transformer_status is None:
                    if skip_reason is None:
                        skip_reason = "Training split unavailable to fit winsor bounds"

                if transformer_status is not None and clipped_series is not None:
                ...
                if requires_training and train_valid_values is not None and not train_valid_values.empty:
                    ...
                    if transformer is not None:
                        ...
                        if mask is not None:
                            ...
                            transformer_status = "fit"
                            skip_reason = None
                            transformer_to_store = transformer
                            metadata_to_store = {
                                "method": method,
                            ...
                            applied_notes.append("Fitted Elliptic Envelope on training split")
                        else:
                            skip_reason = predict_error or "Failed to score rows with Elliptic Envelope"
                    else:
                        skip_reason = fit_error or "Unable to fit Elliptic Envelope"

                if transformer_status != "fit" and stored_transformer is not None:
                    mask, predict_error = _apply_elliptic_envelope_mask(stored_transformer, numeric_series)
                    if mask is not None:
                        mask_to_remove = mask.reindex(working_frame.index, fill_value=False)
                        transformer_status = "reuse"
                        skip_reason = None
                        applied_notes.append("Reused stored Elliptic Envelope model")
                        contamination = float(
                            stored_metadata.get("contamination", contamination) if stored_metadata else contamination
                        )
                    else:
                        skip_reason = predict_error or "Stored Elliptic Envelope failed to score rows"

                if transformer_status is None:
                    if skip_reason is None:
                        skip_reason = "Training split unavailable to fit Elliptic Envelope"
                ...
                if transformer_status is not None and mask_to_remove is not None:
                    ...
                    applied_signal.upper_bound = None
                    applied_signal.affected_rows = affected_rows
                    applied_notes.append(f"Contamination={float(contamination):.4f}")

            else:  # manual (no training required)
                lower_bound = column_params.get("lower_bound", params.get("lower_bound"))
                upper_bound = column_params.get("upper_bound", params.get("upper_bound"))
                summary = _summarize_manual(numeric_series, lower_bound, upper_bound)
                if summary.lower_bound is None and summary.upper_bound is None:
                    skip_reason = "Manual bounds not configured"
                else:
                    mask = pd.Series(False, index=numeric_series.index)
                    if summary.lower_bound is not None:
                        mask |= numeric_series < summary.lower_bound
                    ...
                    applied_signal.lower_bound = summary.lower_bound
                    applied_signal.upper_bound = summary.upper_bound
                    applied_signal.affected_rows = int(mask_to_remove.sum())
                    applied_notes.extend(summary.notes)
                    transformer_status = "direct"
                    skip_reason = None
                    metadata_to_store = {
                        "method": method,
            ...
            if skip_reason:
                signal.skipped_columns.append(f"{column} ({skip_reason})")
                continue

            if transformer_status is None:
                signal.skipped_columns.append(f"{column} (unable to derive parameters)")
                continue

        if mask_to_remove is not None and applied_signal.action == "remove":
            combined_remove_mask |= mask_to_remove
            applied_signal.affected_rows = int(mask_to_remove.sum())

        if clipped_series is not None:
            working_frame.loc[clipped_series.index, column] = clipped_series

        if not applied_signal.notes:
            applied_signal.notes = applied_notes
        else:
            applied_signal.notes.extend(applied_notes)

        applied_columns.append(applied_signal)

        if storage is not None and transformer_status is not None:
            if metadata_to_store is not None or transformer_to_store is not None:
                storage.store_transformer(
                    pipeline_id=pipeline_id,  # type: ignore[arg-type]
                    node_id=str(node_id),
                    transformer_name=transformer_name,
                    transformer=transformer_to_store,
                    column_name=column,
            ...
            elif transformer_status in {"reuse", "direct"}:
