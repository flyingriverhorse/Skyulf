...
def _normalize_problem_type(raw_value: Any) -> str:
...
def _infer_problem_type(series: pd.Series, preferred: str) -> str:
    if preferred in {"classification", "regression"}:
        return preferred
...
def apply_train_model_draft(
    ...
    configured_problem_type = _normalize_problem_type(config.get("problem_type"))
    ...
    problem_type = _infer_problem_type(target_series, configured_problem_type)
