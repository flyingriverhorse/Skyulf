...
async def _resolve_next_version(
    ...
    *,
    ...
    """Return the next model version for the given pipeline/node pair."""

    query: Select[tuple[int]] = select(func.max(TrainingJob.version)).where(
        TrainingJob.pipeline_id == pipeline_id,
