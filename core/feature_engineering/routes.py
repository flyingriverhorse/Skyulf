...
from .nodes.feature_eng.under_resampling import (
    ...
    RESAMPLING_DEFAULT_SAMPLING_STRATEGY as UNDERSAMPLING_DEFAULT_SAMPLING_STRATEGY,
...
from .nodes.feature_eng.casting import (
    COLUMN_CAST_ALIASES,
    ...
    async def ensure_job(
        ...
        *,
        ...
        async with self._lock:
            ...
            if defer_reason:
                reason_parts.append(defer_reason.strip())
            reason_parts.append(f"Background job {job_id} queued for full execution.")
...
def _apply_graph_transformations_before_node(
    ...
    for node_id in execution_order:
        ...
        if catalog_type == "drop_missing_columns":
            working_frame, _, _ = apply_drop_missing_columns(working_frame, node)
            continue

        if catalog_type == "drop_missing_rows":
            working_frame, _, _ = apply_drop_missing_rows(working_frame, node)
            continue

        if catalog_type == "remove_duplicates":
            working_frame, _, _ = apply_remove_duplicates(working_frame, node)
            continue

        if catalog_type == "missing_value_indicator":
            working_frame, _, _ = apply_missing_value_flags(working_frame, node)
            continue

        if catalog_type == "cast_column_types":
            working_frame, _, _ = _apply_cast_column_types(working_frame, node)
            continue

        if catalog_type == "trim_whitespace":
            working_frame, _, _ = apply_trim_whitespace(working_frame, node)
            continue

        if catalog_type == "normalize_text_case":
            working_frame, _, _ = apply_normalize_text_case(working_frame, node)
            continue

        if catalog_type == "replace_aliases_typos":
            working_frame, _, _ = apply_replace_aliases_typos(working_frame, node)
            continue

        if catalog_type == "standardize_date_formats":
            working_frame, _, _ = apply_standardize_date_formats(working_frame, node)
            continue

        if catalog_type == "remove_special_characters":
            working_frame, _, _ = apply_remove_special_characters(working_frame, node)
            continue

        if catalog_type == "replace_invalid_values":
            working_frame, _, _ = apply_replace_invalid_values(working_frame, node)
            continue

        if catalog_type == "feature_math":
            working_frame, _, _ = apply_feature_math(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
        ...
        if catalog_type == "regex_replace_fix":
            working_frame, _, _ = apply_regex_cleanup(working_frame, node)
            continue

        if catalog_type in {"imputation_methods", "advanced_imputer", "simple_imputer"}:
            working_frame, _, _ = _apply_imputation_methods(working_frame, node, pipeline_id=pipeline_id)
            continue

        if catalog_type == "binning_discretization":
            working_frame, _, _ = _apply_binning_discretization(working_frame, node, pipeline_id=pipeline_id)
            continue

        if catalog_type == "skewness_transform":
            working_frame, _, _ = _apply_skewness_transformations(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
        ...
        if catalog_type == "outlier_removal":
            working_frame, _, _ = _apply_outlier_removal(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
        ...
        if catalog_type == "scale_numeric_features":
            working_frame, _, _ = _apply_scale_numeric_features(working_frame, node, pipeline_id=pipeline_id)
            continue

        if catalog_type == "feature_target_split":
            working_frame, _, _ = apply_feature_target_split(working_frame, node)
            continue

        if catalog_type == "train_test_split":
            working_frame, _, _ = apply_train_test_split(working_frame, node)
            
            # After split node, check if target node needs a specific split filtered
            if target_node_id and SPLIT_TYPE_COLUMN in working_frame.columns:
                split_type_filter = _determine_node_split_type(target_node_id, edges, node_map)
                if split_type_filter:
                    original_rows = len(working_frame)
                    working_frame = working_frame[working_frame[SPLIT_TYPE_COLUMN] == split_type_filter].copy()
                    filtered_rows = len(working_frame)
                    logger.info(
                        f"Filtered to '{split_type_filter}' split for node '{target_node_id}': {filtered_rows:,} of {original_rows:,} rows",
                        extra={
        ...
        if catalog_type == "class_undersampling":
            working_frame, _, _ = apply_resampling(working_frame, node)
            continue

        if catalog_type == "class_oversampling":
            working_frame, _, _ = apply_oversampling(working_frame, node)
            continue

        if catalog_type == "label_encoding":
            working_frame, _, _ = apply_label_encoding(working_frame, node, pipeline_id=pipeline_id)
            continue

        if catalog_type == "target_encoding":
            working_frame, _, _ = apply_target_encoding(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
        ...
        if catalog_type == "hash_encoding":
            working_frame, _, _ = apply_hash_encoding(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
        ...
        if catalog_type == "train_model_draft":
            working_frame, _, _ = apply_train_model_draft(working_frame, node)
            continue

        if catalog_type == "ordinal_encoding":
            working_frame, _, _ = apply_ordinal_encoding(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
        ...
        if catalog_type == "one_hot_encoding":
            working_frame, _, _ = apply_one_hot_encoding(working_frame, node, pipeline_id=pipeline_id)
            continue

        if catalog_type == "dummy_encoding":
            working_frame, _, _ = apply_dummy_encoding(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
...
async def _prepare_categorical_recommendation_context(
    *,
    ...
    if graph_node_map or graph_edges or normalized_target_node:
        ...
        try:
            ...
            logger.debug(
                ...
                extra={"result_shape": frame.shape, "result_columns": len(frame.columns)},
            )
    ...
    if quality_payload.get("success"):
...
def _run_pipeline_execution(
    ...
    *,
    ...
    for node_id in execution_order:
        ...
        if _is_node_pending(node):
            applied_steps.append(f"{label}: pending configuration – skipped")
            continue

        if catalog_type in {"dataset", "dataset-source"}:
            applied_steps.append(f"Dataset input '{label}'")
            continue

        if catalog_type == "data_preview":
            applied_steps.append(f"Preview node '{label}': inspection only")
            continue

        if catalog_type == "drop_missing_columns":
            working_frame, summary, signal = apply_drop_missing_columns(working_frame, node)
            if collect_signals:
                signals.drop_missing_columns.append(signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "drop_missing_rows":
            working_frame, summary, signal = apply_drop_missing_rows(working_frame, node)
            if collect_signals:
                signals.drop_missing_rows.append(signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "remove_duplicates":
            working_frame, summary, signal = apply_remove_duplicates(working_frame, node)
            if collect_signals:
                signals.remove_duplicates.append(signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "missing_value_indicator":
            working_frame, summary, signal = apply_missing_value_flags(working_frame, node)
            if collect_signals:
                signals.missing_value_indicator.append(signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "cast_column_types":
            working_frame, summary, signal = _apply_cast_column_types(working_frame, node)
            if collect_signals:
                signals.cast_column_types.append(signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "trim_whitespace":
            working_frame, summary, signal = apply_trim_whitespace(working_frame, node)
            if collect_signals:
                signals.trim_whitespace.append(signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "normalize_text_case":
            working_frame, summary, signal = apply_normalize_text_case(working_frame, node)
            if collect_signals:
                signals.normalize_text_case.append(signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "replace_aliases_typos":
            working_frame, summary, signal = apply_replace_aliases_typos(working_frame, node)
            if collect_signals:
                signals.replace_aliases.append(signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "standardize_date_formats":
            working_frame, summary, signal = apply_standardize_date_formats(working_frame, node)
            if collect_signals:
                signals.standardize_dates.append(signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "remove_special_characters":
            working_frame, summary, signal = apply_remove_special_characters(working_frame, node)
            if collect_signals:
                signals.remove_special_characters.append(signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "replace_invalid_values":
            working_frame, summary, signal = apply_replace_invalid_values(working_frame, node)
            if collect_signals:
                signals.replace_invalid_values.append(signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "feature_math":
            working_frame, summary, feature_math_signal = apply_feature_math(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
            if collect_signals:
                signals.feature_math.append(feature_math_signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "regex_replace_fix":
            working_frame, summary, signal = apply_regex_cleanup(working_frame, node)
            if collect_signals:
                signals.regex_cleanup.append(signal)
            applied_steps.append(summary)
            continue

        if catalog_type in {"imputation_methods", "advanced_imputer", "simple_imputer"}:
            working_frame, summary, imputation_metadata = _apply_imputation_methods(working_frame, node, pipeline_id=pipeline_id)
            if collect_signals:
                signals.imputation.append(imputation_metadata)
            applied_steps.append(summary)
            continue

        if catalog_type == "binning_discretization":
            working_frame, summary, binning_metadata = _apply_binning_discretization(working_frame, node, pipeline_id=pipeline_id)
            if collect_signals:
                signals.binning.append(binning_metadata)
            applied_steps.append(summary)
            continue

        if catalog_type == "skewness_transform":
            working_frame, summary, skewness_metadata = _apply_skewness_transformations(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
            if collect_signals:
                signals.skewness_transform.append(skewness_metadata)
            applied_steps.append(summary)
            continue

        if catalog_type == "scale_numeric_features":
            working_frame, summary, scaling_metadata = _apply_scale_numeric_features(working_frame, node, pipeline_id=pipeline_id)
            if collect_signals:
                signals.scaling.append(scaling_metadata)
            applied_steps.append(summary)
            continue

        if catalog_type == "feature_selection":
            working_frame, summary, selection_signal = apply_feature_selection(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
            if collect_signals:
                signals.feature_selection.append(selection_signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "polynomial_features":
            working_frame, summary, polynomial_signal = apply_polynomial_features(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
            if collect_signals:
                signals.polynomial_features.append(polynomial_signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "outlier_removal":
            working_frame, summary, outlier_signal = _apply_outlier_removal(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
            if collect_signals:
                signals.outlier_removal.append(outlier_signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "feature_target_split":
            working_frame, summary, split_signal = apply_feature_target_split(working_frame, node)
            if collect_signals:
                signals.feature_target_split.append(split_signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "train_test_split":
            working_frame, summary, split_signal = apply_train_test_split(working_frame, node)
            if collect_signals:
                signals.train_test_split.append(split_signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "class_undersampling":
            working_frame, summary, signal = apply_resampling(working_frame, node)
            if collect_signals:
                signals.class_undersampling.append(signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "class_oversampling":
            working_frame, summary, signal = apply_oversampling(working_frame, node)
            if collect_signals:
                signals.class_oversampling.append(signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "label_encoding":
            working_frame, summary, label_metadata = apply_label_encoding(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
            if collect_signals:
                signals.label_encoding.append(label_metadata)
            applied_steps.append(summary)
            continue

        if catalog_type == "target_encoding":
            working_frame, summary, target_metadata = apply_target_encoding(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
            if collect_signals:
                signals.target_encoding.append(target_metadata)
            applied_steps.append(summary)
            continue

        if catalog_type == "hash_encoding":
            working_frame, summary, hash_metadata = apply_hash_encoding(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
            if collect_signals:
                signals.hash_encoding.append(hash_metadata)
            applied_steps.append(summary)
            continue

        if catalog_type == "train_model_draft":
            working_frame, summary, train_metadata = apply_train_model_draft(working_frame, node)
            modeling_metadata = train_metadata
            if collect_signals:
                signals.modeling = train_metadata
            applied_steps.append(summary)
            continue

        if catalog_type == "model_evaluation":
            working_frame, summary, evaluation_signal = apply_model_evaluation(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
            if collect_signals:
                signals.model_evaluation.append(evaluation_signal)
            applied_steps.append(summary)
            continue

        if catalog_type == "model_registry_overview":
            applied_steps.append(f"{label}: model registry view – no transformation applied")
            continue

        if catalog_type == "ordinal_encoding":
            working_frame, summary, ordinal_metadata = apply_ordinal_encoding(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
            if collect_signals:
                signals.ordinal_encoding.append(ordinal_metadata)
            applied_steps.append(summary)
            continue

        if catalog_type == "one_hot_encoding":
            working_frame, summary, one_hot_metadata = apply_one_hot_encoding(
                working_frame, 
                node,
                pipeline_id=pipeline_id
            )
            if collect_signals:
                signals.one_hot_encoding.append(one_hot_metadata)
            applied_steps.append(summary)
            continue

        if catalog_type == "dummy_encoding":
            working_frame, summary, dummy_metadata = apply_dummy_encoding(
                working_frame,
                node,
                pipeline_id=pipeline_id,
            )
            if collect_signals:
                signals.dummy_encoding.append(dummy_metadata)
            applied_steps.append(summary)
            continue

        if catalog_type == "transformer_audit":
            working_frame, summary, audit_signal = apply_transformer_audit(
                working_frame,
                node,
                pipeline_id=pipeline_id,
                node_map=node_map,
            ...
            if collect_signals:
                signals.transformer_audit.append(audit_signal)
            applied_steps.append(summary)
            continue

        applied_steps.append(f"Node '{label}' ({catalog_type}) not executed in preview")
...
async def list_active_datasets(  # pragma: no cover - simple read endpoint
    ...
    return [
        DatasetSourceSummary(
            id=dataset.id,
            source_id=dataset.source_id or str(dataset.id),
            name=dataset.name,
...
async def get_node_catalog() -> List[FeatureNodeCatalogEntry]:
    ...
    def _build_preprocessing_nodes() -> List[FeatureNodeCatalogEntry]:
            ...
            "parameters": [
                {
                    ...
                    "description": "Choose columns to generate missing-value indicator fields (default: columns with missing data).",
                    "type": "multi_select",
                },
                {
                    "name": "flag_suffix",
                    "label": "Indicator suffix",
                    "description": "Suffix appended to the original column name when creating the flag column.",
                    "type": "text",
        ...
        replace_aliases_node: FeatureNodeCatalogEntry = {
            ...
            "parameters": [
                ...
                },
                {
                    "name": "custom_pairs",
                    "label": "Custom pairs",
                    "description": "Provide alias => replacement pairs (one per line). Applies when mode is set to Custom.",
                    "type": "textarea",
        ...
        feature_selection_node: FeatureNodeCatalogEntry = {
            ...
            "description": "Score features and retain the most informative columns using univariate tests or model-based selectors.",
            "inputs": ["dataset"],
            "outputs": ["dataset"],
            "category": "FEATURE ENGINEERING",
            ...
            "parameters": [
                ...
                },
                {
                    "name": "target_column",
                    "label": "Target column",
                    "description": "Supervised selectors require a target column for scoring (e.g., regression/classification target).",
                    "type": "text",
                    "placeholder": "target",
                },
                {
        ...
        feature_math_node: FeatureNodeCatalogEntry = {
            ...
            "description": "Combine columns using arithmetic, ratios, statistics, similarity scores, and datetime extraction steps.",
            "inputs": ["dataset"],
            "outputs": ["dataset"],
            "category": "FEATURE ENGINEERING",
            ...
            "parameters": [
                {
                    ...
                    "description": "Choose whether the pipeline should continue or fail when an operation encounters invalid inputs.",
                    "type": "select",
                    "default": "skip",
                    "options": [
                ...
                },
                {
                    ...
                    "description": "Permit operations to overwrite existing columns when the chosen output already exists.",
                    "type": "boolean",
                    "default": False,
                },
                {
                ...
                },
                {
                    ...
                    "description": "Small constant added to denominators to avoid divide-by-zero when computing ratios.",
                    "type": "number",
                    "default": 1e-9,
                    "min": 0.0,
                    "step": 1e-9,
        ...
        undersampling_node: FeatureNodeCatalogEntry = {
            ...
            "description": "Reduce majority-class rows with random under-sampling to improve balance.",
            "inputs": ["dataset"],
            "outputs": ["dataset"],
            ...
            "parameters": [
                {
                    ...
                    "description": "Choose the sampling approach (under-sampling available now).",
                    "type": "select",
        ...
        oversampling_node: FeatureNodeCatalogEntry = {
            ...
            "description": "Boost minority-class representation with synthetic over-sampling techniques.",
            "inputs": ["dataset"],
            "outputs": ["dataset"],
            "category": "RESAMPLING DATASET",
            ...
            "parameters": [
                {
                    "name": "method",
                    "label": "Resampling method",
                    "description": "Choose the synthetic sampling strategy for balancing classes.",
                    "type": "select",
                ...
                },
                {
                    "name": "k_neighbors",
                    "label": "K-neighbors",
                    "description": "Nearest neighbors considered when synthesising new minority samples.",
                    "type": "number",
        ...
        target_encoding_node: FeatureNodeCatalogEntry = {
            ...
            "description": "Replace categorical values with smoothed averages of a numeric target column.",
            "inputs": ["dataset"],
            "outputs": ["dataset"],
            "category": "Categorical Encoding",
            "tags": ["categorical", "encoding", "feature_engineering"],
            "parameters": [
                {
                    "name": "columns",
                    "label": "Columns to encode",
                    "description": "Select categorical columns or rely on auto-detection when safe.",
                    "type": "multi_select",
                    "source": {
                ...
                },
                {
                    "name": "target_column",
                    "label": "Target column",
                    "description": "Numeric target column used to compute per-category averages (e.g., regression value or 0/1 outcome).",
                    "type": "text",
                    "placeholder": "target",
                },
                {
                ...
                },
                {
                    ...
                    "description": "Higher values pull category means toward the global average to reduce overfitting.",
                    "type": "number",
                ...
                },
                {
                    ...
                    "description": "Assign the global mean to rows where the categorical value is missing.",
                    "type": "boolean",
                    "default": False,
                },
                {
                    ...
                    "description": "Choose whether to assign the global mean or raise an error for categories not observed during fitting.",
                    "type": "select",
        ...
        hash_encoding_node: FeatureNodeCatalogEntry = {
            ...
            "description": "Project categorical values into deterministic hash buckets to bound feature width.",
            "inputs": ["dataset"],
            "outputs": ["dataset"],
            "category": "Categorical Encoding",
            "tags": ["categorical", "encoding", "feature_engineering"],
            "parameters": [
                {
                    "name": "columns",
                    "label": "Columns to encode",
                    "description": "Select categorical columns or rely on auto-detection to handle high-cardinality features.",
                    "type": "multi_select",
                    "source": {
                ...
                },
                {
                    "name": "n_buckets",
                    "label": "Hash buckets",
                    "description": "Number of hash buckets used to encode each column (higher values reduce collisions).",
                    "type": "number",
                ...
                },
                {
                    "name": "output_suffix",
                    "label": "Encoded suffix",
                    "description": "Suffix added to new hashed columns when originals are retained.",
                    "type": "text",
                ...
                },
                {
                    "name": "encode_missing",
                    "label": "Encode missing values",
                    "description": "Assign a dedicated bucket to missing values instead of leaving them null.",
                    "type": "boolean",
                    "default": False,
                },
            ],
        ...
        model_type_options: List[Dict[str, Any]] = []
        ...
        if not model_type_options:
            model_type_options.append({"value": "logistic_regression", "label": "Logistic Regression"})

        preferred_default_model_type = "logistic_regression"
        default_model_type = (
            preferred_default_model_type if any(option["value"] == preferred_default_model_type for option in model_type_options)
            else model_type_options[0]["value"]
        )
        ...
        feature_target_split_node: FeatureNodeCatalogEntry = {
            ...
            "description": "Designate the supervised target column. All remaining columns are treated as features (X).",
            "inputs": ["dataset"],
            "outputs": ["dataset"],
            "category": "MODELING",
            ...
            "parameters": [
                {
                    "name": "target_column",
                    "label": "Target column",
                    "description": "Column treated as the target (y) for downstream modeling.",
                    "type": "text",
                    "placeholder": "target",
                },
            ],
        ...
        train_test_split_node: FeatureNodeCatalogEntry = {
            ...
            "description": "Split dataset into training, testing, and optionally validation sets. Supports stratification.",
            "inputs": ["dataset"],
            ...
            "parameters": [
                ...
                },
                {
                    ...
                    "description": "Proportion of the dataset to include in the validation split (optional).",
                    "type": "number",
                    "default": 0.0,
                    "min": 0.0,
                    "max": 1.0,
        ...
        train_model_draft_node: FeatureNodeCatalogEntry = {
            ...
            "label": "Train model",
            "description": "Validate the pipeline output, launch background training jobs, and expose trained models downstream.",
            "inputs": ["dataset"],
            "outputs": ["models"],
            "category": "MODELING",
            ...
            "parameters": [
                {
                    "name": "target_column",
                    "label": "Target column",
                    "description": "Name of the response column to model (leave blank to map via downstream configuration).",
                    "type": "text",
                    "placeholder": "target",
                },
                {
                    "name": "problem_type",
                    "label": "Problem type",
                    "description": "Choose the expected modeling task for downstream training.",
                    "type": "select",
                ...
                },
                {
                    ...
                    "description": "Select the registered estimator used when launching background jobs.",
                    "type": "select",
                ...
                },
                {
                    ...
                    "description": "Optional JSON object merged with the default parameters for the selected model.",
                    "type": "textarea",
                    "placeholder": "{\n  \"n_estimators\": 200\n}",
                },
                {
                    ...
                    "description": "Run k-fold cross-validation on the training split before finalising the model.",
                    "type": "boolean",
                    "default": False,
                },
                {
                    ...
                    "description": "Choose how folds are generated when cross-validation is enabled.",
                    "type": "select",
                    "default": "auto",
                    "options": [
                        {"value": "auto", "label": "Auto"},
                ...
                },
                {
                    ...
                    "description": "How many folds to use when cross-validation is enabled (minimum 2).",
                    "type": "number",
                    "default": 5,
                    "min": 2.0,
                    "max": 20.0,
                ...
                },
                {
                    ...
                    "description": "Shuffle the training rows before generating cross-validation folds.",
                    "type": "boolean",
                    "default": True,
                },
                {
                    ...
                    "description": "Optional random seed applied when shuffling folds (leave blank for random).",
                    "type": "number",
                    "default": 42,
                    "min": 0.0,
                    "step": 1.0,
                },
                {
                    ...
                    "label": "Refit using",
                    "description": "Choose whether the final model should be refit on the training split only or on training+validation after cross-validation.",
                    "type": "select",
        ...
        model_evaluation_node: FeatureNodeCatalogEntry = {
            ...
            "description": "Review confusion matrices, ROC/PR curves, and residual diagnostics for trained models without leaving the canvas.",
            "inputs": ["dataset"],
            "outputs": ["dataset"],
            "category": "MODELING",
        ...
        model_registry_node: FeatureNodeCatalogEntry = {
            ...
            "description": "Review trained model versions, metrics, and artifacts from the connected training node.",
            "inputs": ["models"],
            "outputs": ["models"],
            "category": "MODELING",
            ...
            "parameters": [
                {
                    ...
                    "description": "Problem type tab that opens by default when viewing the registry.",
                    "type": "select",
                    "default": "auto",
                    "options": [
                ...
                },
                {
                    ...
                    "description": "Optional model template to spotlight initially (e.g. logistic_regression).",
                    "type": "text",
                ...
                },
                {
                    ...
                    "description": "Include queued, running, failed, and cancelled versions in comparisons.",
                    "type": "boolean",
                    "default": True,
                },
            ],
        ...
        hyperparameter_tuning_node: FeatureNodeCatalogEntry = {
            ...
            "description": "Search over candidate hyperparameter combinations using cross-validation and background workers.",
            "inputs": ["dataset"],
            "outputs": ["dataset"],
            "category": "MODELING",
            ...
            "parameters": [
                {
                    "name": "target_column",
                    "label": "Target column",
                    "description": "Name of the response column to optimize against.",
                    "type": "text",
                    "placeholder": "target",
                },
                {
                    "name": "problem_type",
                    "label": "Problem type",
                    "description": "Select the expected modeling task so metrics align.",
                    "type": "select",
                    "default": "classification",
                    "options": [
                        {"value": "classification", "label": "Classification"},
                ...
                },
                {
                    "name": "model_type",
                    "label": "Model template",
                    "description": "Choose which registered estimator to tune.",
                    "type": "select",
                    "default": default_model_type,
                    "options": model_type_options,
                },
                {
                    ...
                    "description": "Optional JSON object merged into the estimator before tuning.",
                    "type": "textarea",
                    "placeholder": "{\n  \"max_iter\": 1000\n}",
                },
                {
                ...
                },
                {
                    ...
                    "description": "JSON object mapping hyperparameter names to lists of candidate values.",
                    "type": "textarea",
                    "placeholder": "{\n  \"C\": [0.1, 1.0, 10.0],\n  \"solver\": [\"lbfgs\", \"saga\"]\n}",
                },
                {
                ...
                },
                {
                    ...
                    "description": "Optional sklearn-compatible scoring string (leave blank for model default).",
                    "type": "text",
                ...
                },
                {
                    "name": "cv_enabled",
                    "label": "Enable cross-validation",
                    "description": "Toggle K-fold cross-validation for evaluating each candidate.",
                    "type": "boolean",
                    "default": True,
                },
                {
                    "name": "cv_strategy",
                    "label": "Cross-validation strategy",
                    "description": "Choose how folds are generated when tuning.",
                    "type": "select",
                    "default": "auto",
                    "options": [
                        {"value": "auto", "label": "Auto"},
                ...
                },
                {
                    "name": "cv_folds",
                    "label": "Number of folds",
                    "description": "How many folds to use when tuning (minimum 2).",
                    "type": "number",
                    "default": 5,
                    "min": 2.0,
                    "max": 20.0,
                ...
                },
                {
                    "name": "cv_shuffle",
                    "label": "Shuffle before splitting",
                    "description": "Shuffle rows before generating folds.",
                    "type": "boolean",
                    "default": True,
                },
                {
                    "name": "cv_random_state",
                    "label": "Shuffle random state",
                    "description": "Optional random seed applied when shuffling folds.",
                    "type": "number",
                    "default": 42,
                    "min": 0.0,
                    "step": 1.0,
        ...
        ordinal_encoding_node: FeatureNodeCatalogEntry = {
            ...
            "description": "Map categorical columns to ordered codes with optional unknown fallbacks.",
            "inputs": ["dataset"],
            "outputs": ["dataset"],
            "category": "Categorical Encoding",
            "tags": ["categorical", "encoding", "feature_engineering"],
            "parameters": [
                ...
                },
                {
                    "name": "encode_missing",
                    "label": "Encode missing values",
                    "description": "Assign the fallback code to missing values instead of leaving them null.",
                    "type": "boolean",
                    "default": False,
                },
                {
                    "name": "handle_unknown",
                    "label": "Handle unseen categories",
                    "description": "Choose whether to assign a fallback code or raise an error for unknown categories.",
                    "type": "select",
                ...
                },
                {
                    "name": "unknown_value",
                    "label": "Fallback code",
                    "description": "Integer code applied to unseen categories or missing values when enabled.",
                    "type": "number",
        ...
        dummy_encoding_node: FeatureNodeCatalogEntry = {
            ...
            "description": "Expand categorical columns while dropping a reference level per feature.",
            "inputs": ["dataset"],
            "outputs": ["dataset"],
            "category": "Categorical Encoding",
            "tags": ["categorical", "encoding", "feature_engineering"],
            "parameters": [
                {
                    "name": "columns",
                    "label": "Columns to encode",
                    "description": "Select categorical columns or rely on auto-detection to capture safe candidates.",
                    "type": "multi_select",
                    "source": {
                ...
                },
                {
                    "name": "auto_detect",
                    "label": "Auto-detect categorical columns",
                    "description": "Automatically include text columns that keep dummy width manageable.",
                    "type": "boolean",
                    "default": False,
                },
                {
                ...
                },
                {
                    ...
                    "description": "Always drop the first dummy column per feature to mitigate multicollinearity.",
                    "type": "boolean",
                    "default": True,
                },
                {
...
async def get_drop_column_recommendations(
    ...
    column_missing_map: Dict[str, float] = {}

    def record_missing_percentage(column: Any, value: Any) -> None:
        ...
        if numeric < 0:
            return
        column_missing_map[name] = max(column_missing_map.get(name, 0.0), float(numeric))

    def build_filter_meta(filter_id: str) -> Dict[str, Optional[str]]:
    ...
    def register_candidate(
        ...
        *,
        ...
        if priority and not entry.get("priority"):
            entry["priority"] = priority

        entry["reasons"].add(reason.strip())

        if signals:
            entry["signals"].update({str(signal).strip() for signal in signals if str(signal).strip()})

        if tags:
            entry["tags"].update({str(tag).strip() for tag in tags if str(tag).strip()})

        if missing_pct is not None:
            try:
                numeric_pct = float(missing_pct)
            except (TypeError, ValueError):  # pragma: no cover - defensive
                numeric_pct = None
            if numeric_pct is not None:
                current = entry.get("missing_percentage")
                entry["missing_percentage"] = max(float(current or 0.0), numeric_pct)
                record_missing_percentage(name, numeric_pct)
    ...
    for record in missing_summary:
        ...
        try:
            numeric_missing = float(missing_pct)
        except (TypeError, ValueError):
            numeric_missing = None

        if numeric_missing is not None:
            missing_values.append(numeric_missing)
            record_missing_percentage(column_name, numeric_missing)
    ...
    for recommendation in eda_recommendations:
    ...
    try:
        suggested_threshold = float(max(20.0, min(95.0, median(missing_values)))) if missing_values else 40.0
    ...
    if graph_node_map or graph_edges or normalized_target_node:
        ...
        else:  # pragma: no cover - preview failed
    ...
    candidate_payload = [
        {
            ...
            "missing_percentage": entry.get("missing_percentage"),
            "priority": entry.get("priority"),
            "signals": sorted(entry.get("signals") or []),
    ...
    if allowed_columns is not None:
        allowed_columns = {column for column in allowed_columns if column}
        candidate_payload = [
            candidate
            for candidate in candidate_payload
            if candidate.get("name") in allowed_columns
        ]
        column_missing_map = {
            name: value
            for name, value in column_missing_map.items()
    ...
    for candidate in candidate_payload:
        ...
        if signals:
            for signal in signals:
                if not signal:
                    continue
                meta = available_filters_map.setdefault(signal, {
                    "id": signal,
                    **build_filter_meta(signal),
                    "count": 0,
                })
                meta["count"] += 1
        else:
    ...
    if orphan_candidates:
        other_meta = available_filters_map.setdefault(
            ...
            {
                "id": "other",
                **build_filter_meta("other"),
                "count": 0,
            },
        )
        other_meta["count"] += orphan_candidates

    available_filters: List[DropColumnRecommendationFilter] = [
        DropColumnRecommendationFilter(**value)
        for value in available_filters_map.values()
        if value.get("count", 0) > 0
    ]
    available_filters.sort(key=lambda item: (-item.count, item.label))
    ...
    for detail in column_details:
        ...
        column_str = str(column_name).strip() if column_name else ""
        if not column_str:
            continue
        all_columns_set.add(column_str)
        detail_missing = (
        ...
        record_missing_percentage(column_str, detail_missing)

    if not all_columns_set:
        sample_preview = quality_report.get("sample_preview") or {}
        for column in sample_preview.get("columns", []) or []:
            column_name = str(column).strip()
            if column_name:
                all_columns_set.add(column_name)

    for candidate in candidate_payload:
        column_name = candidate.get("name")
        if column_name:
            all_columns_set.add(str(column_name))

    if allowed_columns is not None and allowed_columns:
        all_columns_set = {column for column in all_columns_set if column in allowed_columns}
        all_columns_set.update(allowed_columns)

    all_columns = sorted(all_columns_set)

    for column in all_columns:
        column_missing_map.setdefault(column, 0.0)
    ...
    return DropColumnRecommendations(
        ...
        column_missing_map={name: float(value) for name, value in column_missing_map.items()},
    )
...
async def get_one_hot_encoding_recommendations(
    ...
    try:
        ...
        logger.exception("Failed in _prepare_categorical_recommendation_context")
        raise
    ...
    if request.transformations:
        ...
        if isinstance(payload, list):
            for item in payload:
                ...
                if column and method in SKEWNESS_METHODS:
                    selected_methods[column] = method
            ...
            for column, method in payload.items():
                ...
                if column_key and method_key in SKEWNESS_METHODS:
                    selected_methods[column_key] = method_key
        else:  # pragma: no cover - non-supported types
...
async def get_binned_distribution(
    ...
    if graph_node_map or graph_edges or normalized_target_node:
        graph_node_map = _ensure_dataset_node(graph_node_map)
        execution_order = _execution_order(graph_node_map, graph_edges, normalized_target_node)
    ...
    binned_column_metadata = (
        _collect_binned_columns_from_graph(execution_order, graph_node_map, normalized_target_node)
        if graph_node_map
        else {}
    )
    ...
    for column in frame.columns:
        ...
        metadata = binned_column_metadata.get(column_name)
        if metadata:
            candidate_columns[column_name] = metadata
            continue
...
async def preview_pipeline(
    ...
    load_mode = "sample" if effective_sample_size > 0 else "auto"
    frame, preview_meta = await _load_dataset_frame(
        session,
        dataset_source_id,
        sample_size=effective_sample_size,
        execution_mode=load_mode,
        allow_empty_sample=not include_preview_rows,
    )

    preview_rows = int(frame.shape[0])
    preview_total_rows = _coerce_int(
        preview_meta.get("total_rows"),
        preview_rows if include_preview_rows else 0,
    )
    metrics_requested_sample_size = _coerce_int(
        preview_meta.get("sample_size"),
    ...
    full_execution_signal: Optional[FullExecutionSignal] = None
    dataset_already_full = include_preview_rows and effective_sample_size == 0

    if include_preview_rows and dataset_already_full:
        total_rows_actual = preview_total_rows if preview_total_rows > 0 else int(working_frame.shape[0])
        full_execution_signal = FullExecutionSignal(
            status="succeeded",
        ...
        preview_total_rows = total_rows_actual
    elif include_preview_rows:
        total_rows_estimate = preview_total_rows if preview_total_rows > 0 else None
        if total_rows_estimate and total_rows_estimate > FULL_DATASET_EXECUTION_ROW_LIMIT:
            defer_reason = (
            ...
            full_execution_signal = job_signal
    ...
    if include_preview_rows and payload.target_node_id and SPLIT_TYPE_COLUMN in working_frame.columns:
...
async def upsert_pipeline(
    ...
    if pipeline:
        ...
        pipeline.graph = graph_payload
        pipeline.pipeline_metadata = payload.metadata
    else:
...
async def evaluate_trained_model(
    ...
    problem_type = (job.job_metadata or {}).get("resolved_problem_type")
    ...
    metrics_payload = dict(job.metrics or {})
    metrics_payload["evaluation"] = evaluation_record
    job.metrics = metrics_payload

    metadata_payload = dict(job.job_metadata or {})
    ...
    job.job_metadata = metadata_payload
