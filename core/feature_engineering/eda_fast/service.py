...
class FeatureEngineeringEDAService:
    ...
    * :meth:`quality_report` – expose column-level quality metrics used by
    ...
    def _build_quality_report(self, frame: pd.DataFrame) -> Dict[str, Any]:
        column_details = [self._build_column_detail(frame, column) for column in frame.columns]
        column_details = [detail for detail in column_details if detail is not None]
    ...
    def _infer_text_metadata(self, series: pd.Series) -> Tuple[Optional[str], Optional[float]]:
        if not (pd_types.is_object_dtype(series) or pd_types.is_string_dtype(series) or pd_types.is_categorical_dtype(series)):
            return None, None
