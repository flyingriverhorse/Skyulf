    ...
    for profile in profiles:
        ...
        if unique_non_missing <= 1:
            ...
            if text_category in {"identifier", "id", "unique_code"} or profile.unique_ratio >= 0.9:
                ...
                reason = f"More than {absolute_max_limit} distinct values detected; consider alternative encodings."
                selectable = False
                score = 0.1
            elif unique_non_missing > auto_detect_limit and auto_detect_limit > 0:
                status = "high_cardinality"
                reason = (
                    f"High-cardinality text ({unique_non_missing} categories) exceeds the auto-detect limit of {auto_detect_limit}."
                )
                selectable = True
                score = 0.55
            else:
    ...
    for profile in profiles:
        ...
        if unique_non_missing <= 1:
            ...
            if text_category in {"identifier", "id", "unique_code"} or profile.unique_ratio >= 0.85:
                ...
                status = "too_many_categories"
                reason = (
                    f"More than {absolute_max_categories} distinct values detected; one-hot encoding would create excessive features."
                )
                selectable = False
                score = 0.05
                recommended_drop_first = False
                ...
                status = "high_cardinality"
                reason = (
                    f"Very high cardinality ({unique_non_missing} categories) risks sparse, wide matrices. Consider target or frequency encoding."
                )
                selectable = False
                score = 0.25
                recommended_drop_first = False
                ...
                status = "high_cardinality"
                reason = (
                    f"High cardinality ({unique_non_missing} categories) may require dimensionality reduction even though one-hot remains possible."
                )
                selectable = True
                score = 0.45
            else:
    ...
    *,
    ...
    for suggestion in label_suggestions:
        ...
        if status in {"high_cardinality", "too_many_categories"}:
            status = "recommended"
            reason = (
                f"{suggestion.reason} Hash encoding compresses high-cardinality categories into a bounded set of buckets."
            )
            score = max(score, 0.9)
        elif status == "recommended":
            reason = (
                f"{suggestion.reason} Hash encoding offers a numeric representation without manual category mapping."
            )
            score = max(score, 0.6)
        else:
