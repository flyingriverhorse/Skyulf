...
async def get_current_user(
    ...
    try:
        ...
        if token_data.username is None:
            logger.warning("Token data missing username after validation")
            raise credentials_exception
    ...
    if user is None:
        try:
            ...
            logger.error("Failed to hydrate user %s from database: %s", username, exc)
            user = None
...
async def get_optional_user(
    ...
    try:
        ...
        user = USERS_DB.get(username_value)
        if user is None:
            user = await get_user_by_username(username_value)
            if user:
                USERS_DB[username_value] = user
    ...
    **kwargs: Any,
    ...
    try:
        ...
        log_msg = (
            ...
            f"user:{username_str} | IP:{client_ip} | Agent:{user_agent}"
        )
        if details:
            log_msg += f" | {details}"
