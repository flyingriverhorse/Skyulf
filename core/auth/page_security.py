...
def create_page_response(request: Request, template_name: str, extra_context: Dict[str, Any] = None) -> Any:
    """
    ...
    if auth_info["should_redirect"]:
        ...
        if auth_info.get("clear_cookie"):
            ...
            logger.info(f"Cleared invalid cookie during redirect - token status: {auth_info.get('token_status', 'unknown')}")
    ...
    if auth_info.get("clear_cookie"):
        ...
        logger.info(f"Cleared invalid cookie in template response - token status: {auth_info.get('token_status', 'unknown')}")
