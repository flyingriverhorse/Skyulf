...
def _get_template_debug_context() -> dict:
    ...
    try:
        ...
        logger.debug(f"Falling back to safe debug context: {exc}")
        return {"DEBUG": False}
...
def get_templates_from_app(request: Request):
    ...
    try:
        ...
        logger.warning(f"Could not access app state templates: {e}")
    ...
    try:
        ...
        logger.info(f"Creating fallback templates instance from: {templates_dir}")
        return Jinja2Templates(directory=str(templates_dir))
    except Exception as e:
        logger.error(f"Failed to create templates instance: {e}")
        raise
...
def is_browser_request(request: Request) -> bool:
    ...
    if is_browser and not is_api_client:
        logger.debug(f"Browser request detected: UA={user_agent[:50]}, Accept={accept[:50]}")
        return True
        
    # If explicitly requesting JSON as first priority, treat as API request
    if accept.startswith("application/json"):
        logger.debug(f"API request detected: Accept starts with application/json")
        return False
    
    logger.debug(f"Request type: accepts_html={accepts_html}, is_browser={is_browser}, is_api_client={is_api_client}")
    ...
    try:
        if is_browser_request(request):
            ...
            try:
                ...
                context = {
                    "request": request,
                    "error_message": getattr(exc, 'detail', "The page you're looking for doesn't exist. It might have been moved or deleted."),
                    "request_id": getattr(request.state, "request_id", None)
                }
                return templates.TemplateResponse("errors/404.html", context, status_code=404)
            except Exception as template_error:
                logger.error(f"Failed to render 404 template: {template_error}")
        ...
        logger.error(f"Error in 404 handler: {e}", exc_info=True)
    ...
    try:
        ...
        if is_browser_request(request):
            ...
            try:
                ...
                default_msg = "You need to sign in to access this page." if status_code == 401 else "You don't have permission to access this resource."
                context = {
                    "request": request,
                ...
                logger.error(f"Failed to render {status_code} template: {template_error}")
                # Return simple HTML fallback
                title = "Unauthorized" if status_code == 401 else "Forbidden"
                return HTMLResponse(
                    content=f"<html><body><h1>{status_code} - {title}</h1><p>{getattr(exc, 'detail', 'Access denied')}</p></body></html>",
        ...
        logger.error(f"Error in unauthorized handler: {e}", exc_info=True)
        # Fallback to simple JSON response
        return JSONResponse(
            status_code=exc.status_code,
...
async def validation_exception_handler(request: Request, exc: HTTPException) -> Union[HTMLResponse, JSONResponse]:
    ...
    try:
        if is_browser_request(request):
            ...
            try:
                ...
                context = {
                    "request": request,
                    "error_code": "422",
                    "error_message": getattr(exc, 'detail', "The data you submitted couldn't be processed. Please check your input and try again."),
                    "request_id": getattr(request.state, "request_id", None),
                    "error_details": str(getattr(exc, 'detail', ''))
                }
                return templates.TemplateResponse("errors/422.html", context, status_code=422)
            except Exception as template_error:
                logger.error(f"Failed to render 422 template: {template_error}")
                # Return simple HTML fallback
                return HTMLResponse(
                    content="<html><body><h1>422 - Validation Error</h1><p>The data you submitted couldn't be processed.</p></body></html>",
                    status_code=422
                )
        else:
            # Return JSON for API requests
        ...
        logger.error(f"Error in 422 handler: {e}", exc_info=True)
        # Fallback to simple JSON response
        return JSONResponse(
            status_code=422,
...
async def method_not_allowed_exception_handler(request: Request, exc: HTTPException) -> Union[HTMLResponse, JSONResponse]:
    """
    ...
    try:
        if is_browser_request(request):
            ...
            try:
                ...
                context = {
                    "request": request,
                    "error_message": getattr(exc, 'detail', f"The HTTP method {request.method} is not allowed for this endpoint."),
                ...
                logger.error(f"Failed to render 405 template: {template_error}")
                # Return simple HTML fallback
                return HTMLResponse(
                    content=f"<html><body><h1>405 - Method Not Allowed</h1><p>The HTTP method {request.method} is not allowed.</p></body></html>",
                    status_code=405
                )
        else:
            # Return JSON for API requests
        ...
        logger.error(f"Error in method not allowed handler: {e}", exc_info=True)
        # Fallback to simple JSON response
        return JSONResponse(
            status_code=405,
...
async def generic_http_exception_handler(request: Request, exc: Union[HTTPException, StarletteHTTPException]) -> Union[HTMLResponse, JSONResponse]:
    """
    ...
    try:
        ...
        if is_browser_request(request):
            ...
            try:
                ...
                context = {
                    ...
                    "error_code": str(status_code),
                    "error": getattr(exc, 'detail', f"HTTP {status_code} error occurred"),
                    "error_message": getattr(exc, 'detail', f"HTTP {status_code} error occurred"),
                    "request_url": str(request.url),
                    "request_method": request.method,
                    "allowed_methods": request.headers.get("Allow", "GET, POST"),
                    "error_details": str(getattr(exc, 'detail', '')),
                ...
                # Return simple HTML fallback
                return HTMLResponse(
                    content=f"<html><body><h1>{status_code} Error</h1><p>{getattr(exc, 'detail', 'An error occurred')}</p></body></html>",
                    status_code=status_code
                )
        else:
            # Return JSON for API requests
        ...
        logger.error(f"Error in generic HTTP exception handler: {e}", exc_info=True)
        # Fallback to simple JSON response
        return JSONResponse(
