...
class AsyncDeepSeekProvider(AsyncBaseLLMProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        retries_value = (
            ...
            or os.environ.get("DEEPSEEK_MAX_RETRIES")
        )
        backoff_value = (
            config.get("DEEPSEEK_RETRY_BACKOFF_SECONDS")
            or os.environ.get("DEEPSEEK_RETRY_BACKOFF_SECONDS")
        )

        try:
            self.timeout_seconds = max(10, int(float(timeout_value)))
        except (TypeError, ValueError):
            self.timeout_seconds = 90

        try:
            self.max_retries = max(1, int(float(retries_value)))
        except (TypeError, ValueError):
            self.max_retries = 2

        try:
            self.retry_backoff_seconds = max(0.5, float(backoff_value))
        except (TypeError, ValueError):
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                            try:
                                ...
                                self.logger.error(
                                    f"DeepSeek response parsing error: {parse_error}, Response: {data if 'data' in locals() else 'N/A'}"
                                )
                                return self._format_error_response(
                            ...
                            try:
                                ...
                                self.logger.error(
                                    f"DeepSeek API error [{response.status}]: Failed to parse error response. Raw: {response_text[:500]}"
                                )
                                return self._format_error_response(
