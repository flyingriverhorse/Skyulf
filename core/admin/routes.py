...
async def admin_get_logs_tail(
    ...
    try:
        ...
        if not log_file_path.exists():
            log_admin_action(request, current_user, 'APP_LOGS_TAIL', details='no_log_file')
            return {"success": True, "tail": "No application log file found.", "file_info": {"path": str(log_file_path), "exists": False}}
        ...
        log_admin_action(request, current_user, 'APP_LOGS_TAIL', details=f'lines_returned:{len(recent)}')
        ...
        log_admin_action(request, current_user, 'APP_LOGS_TAIL', success=False, details=f'error:{e}')
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to read application log")
...
async def admin_download_log(
    ...
    try:
        ...
        log_file_path = Path('logs') / 'fastapi_app.log'
        if not log_file_path.exists():
            log_admin_action(request, current_user, 'APP_LOGS_DOWNLOAD', details='no_log_file')
            return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content={"success": False, "error": "Log file not found"})

        log_admin_action(request, current_user, 'APP_LOGS_DOWNLOAD')
        return FileResponse(path=str(log_file_path), filename='fastapi_app.log', media_type='text/plain')

    except Exception as e:
        logger.error(f"Error preparing log download: {e}")
        log_admin_action(request, current_user, 'APP_LOGS_DOWNLOAD', success=False, details=f'error:{e}')
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to prepare log download")
