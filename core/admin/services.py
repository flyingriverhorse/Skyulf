    ...
    async def get_user_stats(self) -> Dict[str, Any]:
        ...
        try:
            async for db in get_async_session():
                ...
                self.logger.info(
                    "Retrieved user statistics: %s total users, %s with data sources",
                    total_users,
                    len(data_sources_by_user),
                )
                return stats
    ...
    async def get_user_stats_detailed(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async for db in get_async_session():
                    ...
                    if getattr(row, 'data_source_count', 0) > 0:
                        ...
                        for source in user_sources:
                            try:
                                ...
                                self.logger.warning(
                                    "Error calculating size for source %s: %s",
                        ...
                        enhanced_data_sources_by_user[row.username] = {
                            ...
                            'size': size_formatted,
                            'last_activity': (
                                row.last_data_activity.strftime('%Y-%m-%d')
                                if row.last_data_activity
                                else 'Never'
                            ),
                        }
                ...
                enhanced_stats = {
                    **base_stats,
                    ...
                    "data_sources_summary": {
                        "total_sources": (
                            sum(data_sources_by_user.values())
                            if isinstance(data_sources_by_user, dict)
                            else 0
                        ),
                        "average_per_user": (
                            round(sum(data_sources_by_user.values()) / total_users, 1)
                            if total_users > 0 and isinstance(data_sources_by_user, dict)
                            else 0
                        ),
                ...
                self.logger.info(
                    "Retrieved detailed user statistics with %s users and %s online",
                    len(user_table_data),
                    online_count,
                )
...
class AsyncAdminSystemService:
        ...
        try:
            ...
            try:
                ...
                info = {
                    ...
                    "storage": {
                        "uploads": {
                            "size_bytes": uploads_info['size'],
                            "size_formatted": _format_size(uploads_info['size']),
                        },
                        "cache": {
                            "size_bytes": cache_info['size'],
                            "size_formatted": _format_size(cache_info['size']),
                        },
                        "logs": {
                            "size_bytes": logs_info['size'],
                            "size_formatted": _format_size(logs_info['size']),
                        },
                        "exports": {
                            "size_bytes": exports_info['size'],
                            "size_formatted": _format_size(exports_info['size']),
                        },
                        "total_files": (
                            uploads_info['count']
                            + cache_info['count']
    ...
    async def get_system_health_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                "Generated system health report - Status: %s",
                report['status'],
            )
            return report
    ...
    async def get_database_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning("Database auto-initialization failed: %s", e)
            status["auto_init"] = {
                "performed": False,
                "results": {"error": str(e)}
            }
        ...
        try:
            ...
            if conn is None:
                status["postgres"] = {
                    ...
                    "count": 0,
                    "error": (
                        "PostgreSQL not configured (missing DB_USER, DB_PASSWORD, DB_HOST, or "
                        "DB_NAME in environment)"
                    ),
                }
                return status
            ...
            self.logger.info(
                "PostgreSQL connected with %s records",
                postgres_count,
            )
        ...
        self.logger.info(
            "Database status check completed - SQLite: %s records, PostgreSQL: %s records",
            status['sqlite']['count'],
            status['postgres']['count'],
        )
        return status
    ...
    async def get_database_contents(self, database_type: str = "current", limit: int = 100) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                "Retrieved %s database contents: %s records",
                database_type,
                contents['tables']['data_sources']['count'],
            )
            return contents
