...
class ModelRegistryService:
    ...
    async def get_registry_stats(session: AsyncSession) -> RegistryStats:
        ...
        tune_count = await session.scalar(select(func.count(HyperparameterTuningJob.id)).where(HyperparameterTuningJob.status == "completed"))
        
        # Count active deployments
        deploy_count = await session.scalar(select(func.count(Deployment.id)).where(Deployment.is_active == True))
    ...
    async def list_models(session: AsyncSession, skip: int = 0, limit: int = 20) -> List[ModelRegistryEntry]:
        ...
        deployments = await session.execute(select(Deployment))
        ...
        data_sources = await session.execute(select(DataSource))
        ds_map = {ds.source_id: ds.name for ds in data_sources.scalars().all()}
        ...
        train_jobs = train_jobs.scalars().all()
        ...
        tune_jobs = tune_jobs.scalars().all()
        
        # Group by (model_type, dataset_source_id)
        grouped: Dict[Tuple[str, str], List[ModelVersion]] = {}
        
        for job in train_jobs:
            m_type = job.model_type or "unknown"
            ds_id = job.dataset_source_id or "unknown"
            key = (m_type, ds_id)
        ...
        for (m_type, ds_id), versions in grouped.items():
            ...
            ds_name = ds_map.get(ds_id, f"Dataset {ds_id}")
        ...
        results.sort(key=lambda x: x.latest_version.created_at if x.latest_version and x.latest_version.created_at else datetime.min, reverse=True)
