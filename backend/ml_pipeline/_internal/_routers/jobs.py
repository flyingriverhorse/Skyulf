...
from backend.realtime.trial_buffer import get_trials
...
async def get_job_trials(job_id: str):
    """Snapshot of completed trials so far, for charts opened mid-run.

    The ``/ws/jobs`` broadcast only reaches already-connected clients; this
    backfills the trials a late opener missed. Empty once the job's buffer
    has been evicted — terminal jobs redraw from persisted ``metrics.trials``.
    """
    trials = get_trials(job_id)
    metric = next((t["metric"] for t in reversed(trials) if t["metric"]), None)
    return {"trials": trials, "metric": metric}
