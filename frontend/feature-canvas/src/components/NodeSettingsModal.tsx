...
export const NodeSettingsModal: React.FC<NodeSettingsModalProps> = ({
  ...
  }, [isOutlierNode, refreshOutliers, refreshPreview]);
  ...
  const handleBinningStrategyChange = useCallback((strategy: BinningStrategy) => {
    setConfigState((previous) => {
      const currentStrategy = typeof previous?.strategy === 'string' ? previous.strategy : undefined;
      ...
      if (strategy !== 'custom') {
        if (Object.prototype.hasOwnProperty.call(next, 'custom_bins')) {
          delete next.custom_bins;
        }
        if (Object.prototype.hasOwnProperty.call(next, 'custom_labels')) {
  ...
  const handleBinningDuplicatesChange = useCallback((value: 'raise' | 'drop') => {
    setConfigState((previous) => {
      const current = previous?.duplicates === 'drop' ? 'drop' : 'raise';
      if (current === value) {
        return previous;
      }
      return {
  ...
  const handleKbinsEncodeChange = useCallback((value: KBinsEncode) => {
    setConfigState((previous) => {
      const raw = typeof previous?.kbins_encode === 'string' ? previous.kbins_encode : '';
      const current = (['ordinal', 'onehot', 'onehot-dense'] as KBinsEncode[]).includes(raw as KBinsEncode)
        ? (raw as KBinsEncode)
        : 'ordinal';
      if (current === value) {
        return previous;
  ...
  const handleKbinsStrategyChange = useCallback((value: KBinsStrategy) => {
    setConfigState((previous) => {
      const raw = typeof previous?.kbins_strategy === 'string' ? previous.kbins_strategy : '';
      const current = (['uniform', 'quantile', 'kmeans'] as KBinsStrategy[]).includes(raw as KBinsStrategy)
        ? (raw as KBinsStrategy)
        : 'quantile';
      if (current === value) {
        return previous;
      }
  ...
  useEffect(() => {
    ...
    setConfigState((previous) => {
      const skippedSet = new Set(ensureArrayOfString(previous.skipped_columns));
      const currentColumnsSet = new Set(ensureArrayOfString(previous.columns));
      const nextMethods =
        previous.column_methods &&
        typeof previous.column_methods === 'object' &&
        !Array.isArray(previous.column_methods)
      ...
      scalingRecommendations.forEach((entry) => {
        const columnKey = String(entry.column ?? '').trim();
        if (!columnKey || skippedSet.has(columnKey)) {
          return;
        }
        if (!currentColumnsSet.has(columnKey)) {
          currentColumnsSet.add(columnKey);
          updated = true;
        }
        const recommendedMethod = entry.recommended_method;
        if (
          SCALING_METHOD_ORDER.includes(recommendedMethod) &&
          !Object.prototype.hasOwnProperty.call(nextMethods, columnKey) &&
          recommendedMethod !== scalingConfig.defaultMethod
        ) {
          nextMethods[columnKey] = recommendedMethod;
          updated = true;
        }
      ...
      return {
        ...previous,
        columns: Array.from(currentColumnsSet).sort((a, b) => a.localeCompare(b)),
        column_methods: nextMethods,
      };
    });
  }, [
    scalingAutoDetectEnabled,
    scalingConfig.defaultMethod,
    scalingRecommendations,
    setConfigState,
  ]);
  ...
  const busyLabel = useMemo(() => {
    ...
    if (isFetchingSkewness) {
      return 'Loading skewness insights…';
    }
    ...
    (parameter: FeatureNodeParameter) => {
      ...
      const displayedColumns = (() => {
        if (isBinningColumnsParameter) {
          return availableColumns.filter((column) => !binningExcludedColumns.has(column));
        }
      ...
      const renderedColumnOptions = (() => {
        if (isBinningColumnsParameter) {
          return filteredColumnOptions.filter((column) => !binningExcludedColumns.has(column));
        }
        if (isScalingColumnsParameter) {
        ...
        <div className="canvas-modal__body">
          ...
          {isBinningNode && (
            <BinNumericColumnsSection
              ...
              columnsParameter={binningColumnsParameter}
              renderMultiSelectField={renderMultiSelectField}
              fieldIds={binningFieldIds}
              customEdgeDrafts={binningCustomEdgeDrafts}
              customLabelDrafts={binningCustomLabelDrafts}
              onStrategyChange={handleBinningStrategyChange}
              ...
              onDuplicatesChange={handleBinningDuplicatesChange}
              ...
              onKbinsEncodeChange={handleKbinsEncodeChange}
              onKbinsStrategyChange={handleKbinsStrategyChange}
            />
          )}
          {isSkewnessNode && (
