...
export type FetchBinningRecommendationsOptions = {
  sampleSize?: number;
  graph?: FeatureGraph | null;
  targetNodeId?: string | null;
};
...
export async function generatePipelineId(
  ...
  // Hash the sanitized graph so distinct pipeline shapes map to unique identifiers while versions stay sequential server-side.
