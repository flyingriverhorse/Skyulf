...
interface OneHotEncodingConfig {
  columns: string[];
}

const OneHotEncodingSettings: React.FC<{ config: OneHotEncodingConfig; onChange: (c: OneHotEncodingConfig) => void; nodeId?: string }> = ({
  config,
  onChange,
  nodeId,
}) => {
    ...
    <div className="p-4 space-y-4">
      ...
      <div>
        ...
        <p className="text-[10px] text-muted-foreground mt-2">
          Creates binary columns for each category. Best for low-cardinality features.
        </p>
      </div>
    </div>
  );
...
export const OneHotEncodingNode: NodeDefinition<OneHotEncodingConfig> = {
  type: 'one_hot_encoding',
  label: 'One-Hot Encoding',
  category: 'Preprocessing',
  description: 'Convert categorical variables into binary columns.',
  icon: Binary,
  inputs: [{ id: 'in', label: 'Data', type: 'dataset' }],
  outputs: [{ id: 'out', label: 'Encoded Data', type: 'dataset' }],
  settings: OneHotEncodingSettings,
  validate: (config) => ({ 
