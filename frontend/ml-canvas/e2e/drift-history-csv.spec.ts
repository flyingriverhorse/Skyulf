...
test('keeps real producer PSI aligned across table, sparse history and CSV', async ({ page }) => {
  ...
  const psiInput = page.getByRole('spinbutton', { name: 'PSI', exact: true });
  await expect(page.getByText('Effective PSI: 0.5 (override)', { exact: true })).toBeVisible();
  await psiInput.fill('0');
  await expect(page.getByText('Effective PSI: 0 (override)', { exact: true })).toBeVisible();
  await expect(sparkline).toHaveAccessibleName(/threshold 0$/);
  await psiInput.fill('-1');
  await expect(page.getByRole('alert')).toContainText('Not applied');
  await expect(sparkline).toHaveAccessibleName(/threshold 0$/);
  await psiInput.fill('5');
  await expect(page.getByText('Effective PSI: 5 (override)', { exact: true })).toBeVisible();
  await expect(sparkline.locator('circle').last()).toHaveAttribute('fill', '#22c55e');
  await psiInput.fill('');
  await expect(page.getByText('Effective PSI: 0.2 (saved report)', { exact: true })).toBeVisible();
  await expect(sparkline).toHaveAccessibleName(/threshold 0.2$/);
  await expect(sparkline.locator('circle').last()).toHaveAttribute('fill', '#ef4444');
  await page.getByRole('button', { name: 'Reset defaults', exact: true }).click();
  await expect(psiInput).toHaveValue('0.2');
  await expect(page.getByText('Effective PSI: 0.2 (override)', { exact: true })).toBeVisible();
  await page.getByRole('button', { name: 'Remove overrides', exact: true }).click();
  await expect(psiInput).toHaveValue('');
  await expect(page.getByText('Effective PSI: 0.2 (saved report)', { exact: true })).toBeVisible();
});
