...
def generate_dataset(n_rows: int = 100_000):
    ...
    print(
        f"Dataset Size in Memory (Pandas): {df_pd.memory_usage(index=True).sum() / 1024**2:.2f} MB"
    )
    print("-" * 60)
    return df_pd, df_pl
...
def run_pipeline(df, engine_name: str):
    ...
    df = measure(
        "Scaling", StandardScalerCalculator, StandardScalerApplier, {"columns": ["income", "score"]}
    )
    ...
    df = measure(
        "Encoding", OneHotEncoderCalculator, OneHotEncoderApplier, {"columns": ["color", "zone"]}
    )
...
def main():
    ...
    print(
        f"{'TOTAL PIPELINE':<20} | {pd_total:<12.4f} | {pl_total:<12.4f} | {speedup_total:<9.2f}x"
    )
    print("=" * 75)
