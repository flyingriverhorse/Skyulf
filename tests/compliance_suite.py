...
def test_onehot_encoder_parity():
    ...
    data = {"cat": ["a", "b", "a", "c", "b"], "num": [1, 2, 3, 4, 5]}
    df_pd = pd.DataFrame(data)
    df_pl = pl.DataFrame(data)
...
def test_dummy_encoder_parity():
    ...
    pd.testing.assert_frame_equal(
        res_pd.sort_index(axis=1).astype(int), res_pl_pd.sort_index(axis=1).astype(int)
    )
...
def test_knn_imputer_parity():
    ...
    data = {"a": [1.0, 2.0, None, 4.0, 5.0], "b": [10.0, 20.0, 30.0, 40.0, 50.0]}
    df_pd = pd.DataFrame(data)
    df_pl = pl.DataFrame(data)
...
def test_iqr_parity():
    ...
    pd.testing.assert_frame_equal(
        res_pd.reset_index(drop=True), res_pl_pd.reset_index(drop=True), check_dtype=False
    )
...
def test_casting_parity():
    ...
    config = {
        "columns": ["a", "b", "c"],
        "target_type": "float",
    }  # This config is for Calculator fit, but CastingCalculator fit just returns config
...
def test_text_cleaning_parity():
    ...
    data = {"text": ["  Hello World  ", "Foo Bar", "123 Go!"], "mixed": ["UPPER", "lower", "MiXeD"]}
    df_pd = pd.DataFrame(data)
    df_pl = pl.DataFrame(data)
...
def test_drop_missing_rows_parity():
    ...
    data = {"a": [1, 2, None, 4], "b": [1, None, None, 4]}
    df_pd = pd.DataFrame(data)
    df_pl = pl.DataFrame(data)
    ...
    pd.testing.assert_frame_equal(
        res_pd_t.reset_index(drop=True),
        res_pl_t.to_pandas().reset_index(drop=True),
        check_dtype=False,
    )
