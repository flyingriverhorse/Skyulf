...
class TestPolarsPreprocessingComprehensive(unittest.TestCase):
    ...
    def test_drop_and_missing_nodes(self):
        ...
        # 3. Drop Missing Rows
        res, _ = self._apply_calc_applier(
            DropMissingRowsCalculator, DropMissingRowsApplier, self.df, {"subset": ["F_missing"]}
        )
        # Should drop 2 rows
        ...
        # 4. Missing Indicator
        res, _ = self._apply_calc_applier(
            MissingIndicatorCalculator, MissingIndicatorApplier, self.df, {"columns": ["F_missing"]}
        )
    ...
    def test_encoding_nodes(self):
        ...
        # 2. Ordinal
        res, _ = self._apply_calc_applier(
            OrdinalEncoderCalculator, OrdinalEncoderApplier, df_enc, {"columns": ["C_txt"]}
        )
    ...
    def test_more_imputation_nodes(self):
        ...
        res, _ = self._apply_calc_applier(
            KNNImputerCalculator,
            KNNImputerApplier,
            self.df.select(["A_float", "B_int", "F_missing"]),
            {"n_neighbors": 2},
        )
    ...
    def test_more_outlier_nodes(self):
        ...
        res, _ = self._apply_calc_applier(
            ZScoreCalculator, ZScoreApplier, self.df, {"columns": ["G_outlier"], "threshold": 1.0}
        )
        self.assertTrue(len(res) < len(self.df))
    ...
    def test_scaling_nodes(self):
        ...
        # 1. Standard
        res, _ = self._apply_calc_applier(
            StandardScalerCalculator, StandardScalerApplier, self.df, {"columns": ["B_int"]}
        )
        # Standardize B_int
        ...
        # 2. MinMax
        res, _ = self._apply_calc_applier(
            MinMaxScalerCalculator, MinMaxScalerApplier, self.df, {"columns": ["B_int"]}
        )
    ...
    def test_transformation_nodes(self):
        ...
        res, _ = self._apply_calc_applier(
            PowerTransformerCalculator, PowerTransformerApplier, df_num, {"method": "yeo-johnson"}
        )
