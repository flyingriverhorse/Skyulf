...
def test_text_cleaning_trim():
    ...
    config = {"columns": ["A"], "operations": [{"op": "trim", "mode": "both"}]}
...
def test_text_cleaning_case():
    ...
    config = {"columns": ["A"], "operations": [{"op": "case", "mode": "upper"}]}
...
def test_text_cleaning_remove_special():
    ...
    config = {
        "columns": ["A"],
        "operations": [{"op": "remove_special", "mode": "keep_alphanumeric"}],
    }
...
def test_value_replacement_simple():
    df = pd.DataFrame({"A": ["Yes", "No", "Maybe"], "B": [1, 2, 3]})

    config = {"columns": ["A"], "mapping": {"Yes": 1, "No": 0}}
...
def test_value_replacement_numeric():
    ...
    config = {"columns": ["A"], "mapping": {1: 10, 2: 20}}
...
def test_value_replacement_replacements_list():
    ...
    config = {
        "columns": ["A"],
        "replacements": [{"old": 1, "new": 100}, {"old": 2, "new": 200}],
    }
...
def test_invalid_value_replacement_negative():
    ...
    config = {"columns": ["A"], "mode": "negative_to_nan"}
