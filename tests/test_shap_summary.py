"""Unit tests for `ArtifactsMixin._extract_shap_explanation` (backend data-shape resolution).
...
def test_extract_shap_explanation_with_dataframe(host: _Host, classification_frame: pd.DataFrame):
    ...
    fake_result = {"feature_names": ["a", "b", "c"], "mean_abs_importance": {}, "samples": []}

    with patch(
        "backend.ml_pipeline._execution.engine._artifacts.compute_shap_explanation"
    ) as mock_compute:
        mock_compute.return_value = fake_result
        result = host._extract_shap_explanation(model, classification_frame, "target")
...
def test_extract_shap_explanation_with_split_dataset(
    ...
    with patch(
        ...
        mock_compute.return_value = fake_result
        result = host._extract_shap_explanation(model, split, "target")
...
def test_extract_shap_explanation_with_xy_tuple(host: _Host, classification_frame: pd.DataFrame):
    ...
    with patch(
        ...
        mock_compute.return_value = {"feature_names": [], "mean_abs_importance": {}, "samples": []}
        host._extract_shap_explanation(model, (X, y), "target")

    passed_X = mock_compute.call_args[0][1]
    assert list(passed_X.columns) == ["a", "b", "c"]


def test_extract_shap_explanation_unwraps_tuning_tuple(
    host: _Host, classification_frame: pd.DataFrame
):
    ...
    with patch(
        ...
        mock_compute.return_value = {"feature_names": [], "mean_abs_importance": {}, "samples": []}
        host._extract_shap_explanation((model, {"best_params": {}}), classification_frame, "target")

    passed_model = mock_compute.call_args[0][0]
    assert passed_model is model


def test_extract_shap_explanation_returns_none_for_unresolvable_data(host: _Host):
    """Data that can't be resolved to a feature frame short-circuits to `None`."""
    result = host._extract_shap_explanation(RandomForestClassifier(), object(), "target")

    assert result is None


def test_extract_shap_explanation_end_to_end_with_real_model(
    host: _Host, classification_frame: pd.DataFrame
):
