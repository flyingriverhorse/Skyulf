...
def test_custom_binning(sample_data):
    ...
    assert (
        result["C_custom"].iloc[4] == 0.0
    )  # 50 (include_lowest=True usually handles left edge, but pd.cut default right=True)
...
def test_multiple_columns_mixed_strategies(sample_data):
    ...
    config = {"columns": ["A", "B"], "strategy": "equal_width", "n_bins": 3}
