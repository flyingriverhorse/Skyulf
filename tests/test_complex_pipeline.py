...
def test_complex_pipeline_flow(complex_data, artifact_store):
    ...
    impute_node_cat = StatefulTransformer(
        impute_calc, impute_app, artifact_store, "node_3_impute_cat"
    )
    impute_node_val = StatefulTransformer(
        impute_calc, impute_app, artifact_store, "node_4_impute_val"
    )
