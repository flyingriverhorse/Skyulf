...
async def test_artifact_routing_logic(tmp_path):
    ...
    with (
        patch("backend.ml_pipeline.tasks.get_db_session") as mock_get_session,
        ...
        patch("backend.ml_pipeline._services.pipeline_execution_service.PipelineEngine"),
        patch("os.makedirs"),
    ):
            ...
            "nodes": [
                {
                    ...
                    "params": {"dataset_id": "s3://bucket/data.csv"},
                    "inputs": [],
                }
            ],
        }
            ...
            "nodes": [
                {
                    "node_id": "n1",
                    "step_type": "data_loader",
                    "params": {"dataset_id": "/local/data.csv"},
                    "inputs": [],
                }
            ],
        }
